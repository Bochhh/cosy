...
public class UserController {
    ...
    @GetMapping("/brokers")
    @PreAuthorize("hasAuthority('ADMIN')")
    public ResponseEntity<CustomPageResponse<Broker>> getAllBrokers(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size) {
        return ResponseEntity.ok(userService.getAllBroker(page, size));
    }
    ...
    public ResponseEntity<BrokerDetailsDTO> getBrokerById(@PathVariable Long brokerId) {
        return ResponseEntity.ok(userService.getBrokerById(brokerId));
    }
