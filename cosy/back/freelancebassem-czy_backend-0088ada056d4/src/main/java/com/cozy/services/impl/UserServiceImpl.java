...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public CustomPageResponse<Broker> getAllBroker(int page, int size) {
        PageRequest pageRequest = PageRequest.of(page, size);
        Page<Broker> brokers= brokerRepository.findAll(pageRequest);
        CustomPageable customPageable = new CustomPageable(
                brokers.getPageable().getPageSize(),
                brokers.getPageable().getPageNumber(),
                brokers.getTotalElements(),
                brokers.getTotalPages()
        );
        return new CustomPageResponse<>(
                brokers.getContent(),
                customPageable
    ...
    @Override
    public BrokerDetailsDTO getBrokerById(Long brokerId) {
        Broker broker =  brokerRepository.findById(brokerId)
                .orElseThrow(() -> new NotFoundException("Broker not found with ID: " + brokerId));

        long propertiesWithBrokerStatus = propertyRepository.countPropertiesByStatusAndBrokerId( PropertyStatus.WITH_BROKER, brokerId);
        long propertiesPublished = propertyRepository.countPropertiesByStatusAndBrokerId(PropertyStatus.PUBLISHED, brokerId);
        BrokerDetailsDTO responseDTO = new BrokerDetailsDTO();
        responseDTO.setId(broker.getId());
        responseDTO.setFirstname(broker.getFirstname());
        responseDTO.setLastname(broker.getLastname());
        responseDTO.setEmail(broker.getEmail());
        responseDTO.setPhoneNumber(broker.getPhoneNumber());
        responseDTO.setPropertiesWithBroker(propertiesWithBrokerStatus);
        responseDTO.setPropertiesPublished(propertiesPublished);
